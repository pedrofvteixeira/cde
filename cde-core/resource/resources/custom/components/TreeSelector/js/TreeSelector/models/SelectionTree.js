'use strict';
(function(_, TreeSelector, Models, Tree) {
  'use strict';
  ...
  return Models.SelectionTree = Tree.extend({
    ...
    setSelection: function(newState) {
      ...
      if (newState !== TreeSelector.Enum.select.SOME) {
        if (this.children()) {
    ...
    setSelectedItems: function(idList) {
      ...
      flatTree.filter(function(m) {
        ...
        if (_.contains(idList, id)) {
          return m.setSelection(TreeSelector.Enum.select.ALL);
        } else {
          return m.setSelection(TreeSelector.Enum.select.NONE);
        }
      });
      flatTree.filter(function(m) {
        return m.children() != null;
    ...
    _getSelectionSnapshot: function() {
      ...
      selectionSnapshot = {
        ...
        some: flatTree.filter(function(m) {
          return m.getSelection() === TreeSelector.Enum.select.SOME;
        }),
    ...
    update: function() {
      ...
      this.root().updateCountOfItems('numberOfSelectedItems', function(model) {
        if (model.getSelection() === TreeSelector.Enum.select.ALL) {
          return 1;
        } else {
          return 0;
        }
...
})(_, TreeSelector, TreeSelector.Models, TreeSelector.Models.Tree);
